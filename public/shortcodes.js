export function Shortcodes() {
      ...
      accordion.addEventListener('click', event => {
          if (event.target.tagName === 'DT') {
              ...
              if (dt.classList.contains('accordion-active')) {
                  ...
                  slideDown(dd);
                  dt.parentNode.querySelectorAll('dd.accordion-content').forEach(otherDd => {
                      if (otherDd !== dd) {
                          slideUp(otherDd);
                          otherDd.previousElementSibling.classList.remove('accordion-active');
                      }
                  });
              }
  ...
  if (document.getElementById('share')) {
      ...
      document.querySelectorAll('.jssocials-share').forEach(share => {
          const wrapper = document.createElement('div');
          wrapper.className = 'parallax-wrap';
          const element = document.createElement('div');
          element.className = 'parallax-element';
          wrapper.appendChild(element);
          share.parentNode.insertBefore(wrapper, share);
  ...
  if (document.querySelector('.random-collage-wrap')) {
      if (window.innerWidth >= 1024) {
          document.querySelectorAll(".random-collage .rc-slide .item-wrap-image").forEach(item => {
              item.addEventListener('mouseenter', () => {
                  gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.2, borderColor: document.body.dataset.primaryColor, backgroundColor: document.body.dataset.primaryColor});
  ...
  if (document.querySelector('.has-hover-image')) {
      const parentRow = document.querySelector('.has-hover-image').closest('.content-row');
      ...
      if (document.body.classList.contains("smooth-scroll")) {
          ...
          const scrollbar = Scrollbar.init(elem, {renderByPixels: true, damping:0.1});
      }
      ...
      class HoverImgFx1 {
          ...
          showImage() {
            ...
            gsap.to.killTweensOf(this.DOM.revealImg);
              ...
              .add('begin')
              .add(new gsap.to(this.DOM.revealInner, 0.3, {
                  ease: Sine.easeOut,
                  startAt: {x: '-100%'},
                  x: '0%'
              }), 'begin')
              .add(new gsap.to(this.DOM.revealImg, 0.3, {
                  ease: Sine.easeOut,
                  startAt: {x: '100%'},
                  x: '0%'
              }), 'begin');
